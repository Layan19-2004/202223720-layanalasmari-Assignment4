...
// ===== Assignment 4 Upgrades =====

// Auto-load GitHub repositories when page loads
...
// ===== Save Last Project Filter =====
filterButtons.forEach((button) => {
    button.addEventListener("click", () => {
        localStorage.setItem("lastProjectFilter", button.dataset.filter);
    });
});

function applySavedProjectFilter() {
    const savedFilter = localStorage.getItem("lastProjectFilter");
...
window.addEventListener("load", applySavedProjectFilter);